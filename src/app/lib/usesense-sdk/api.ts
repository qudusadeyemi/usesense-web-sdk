...
export class UseSenseAPI {
  ...
  private async handleErrorResponse(response: Response): Promise<Error> {
    ...
    console.warn('[UseSense SDK] Raw 401 response body:', rawBody);
    ...
    // Gateway errors: no edge-function-specific code, and message matches Supabase patterns
    const msgLower = serverMessage.toLowerCase();
    const isSupabaseGatewayError =
      response.status === 401 &&
      !serverCode && // Edge Functions set their own codes; Supabase gateway does not
      (msgLower.includes('invalid api key') ||
       msgLower.includes('invalid claim') ||
       msgLower.includes('jwt') ||
       msgLower.includes('apikey') ||
       // Supabase hint field is a strong gateway signal
       !!errorData.hint);
    ...
    switch (response.status) {
      ...
      case 401:
        if (serverCode === 'session_expired') {
          ...
          // Edge Function returned a message — show it directly
          errorMessage = `Authentication failed: ${serverMessage}`;
          errorCode = serverCode || 'UNAUTHORIZED';
        } else {
          // No message at all — include raw body for debugging
  ...
  private createMockSessionResponse(request: CreateSessionRequest): CreateSessionResponse {
