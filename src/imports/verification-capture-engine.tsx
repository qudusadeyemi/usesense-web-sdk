 ...
 *
 * Extracted from the SDK API Tester's battle-tested capture logic.
 * This component is the "Web SDK" that runs the full capture pipeline:
...
import { Camera, ArrowLeft, ArrowRight, ArrowUp, ArrowDown, Crosshair, Sun, Moon, Aperture, AlertTriangle } from 'lucide-react';
...
// Matches the signal universe defined in rule-builder.tsx DeepSense categories.
...
async function collectWebIntegritySignals(): Promise<Record<string, any>> {
  ...
  // ── Derived permission booleans (for native scoring path compatibility) ──
...
export function VerificationCaptureEngine({
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Main capture flow — mirrors the SDK tester's proven pipeline
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const runCaptureFlow = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        // Wait for the video to actually produce frames (videoWidth > 0)
        // to avoid capturing black frames before the camera warms up
      ...
      // 1b. Camera succeeded → update permission signals
      // collectWebIntegritySignals() ran before getUserMedia, so permissions_state.camera
      // was 'prompt'. Now that the user granted, patch the signals.
      ...
      // Update permission signals on denial too
      if (webIntegrityRef.current) {
        if (!webIntegrityRef.current.permissions_state) webIntegrityRef.current.permissions_state = {};
      ...
      return; // don't call onError yet — let user retry
    }
    ...
    if (challengeSpec?.type === 'head_turn' && challengeSpec.sequence) {
      ...
      for (const step of challengeSpec.sequence) {
        ...
        // Ensure minimum frames per step
    ...
    try {
      ...
      if (ci) {
        // Camera resolution (actual video feed dimensions)
        const vw = videoRef.current?.videoWidth || 640;
        const vh = videoRef.current?.videoHeight || 480;
        ci.camera_resolution = `${vw}x${vh}`;
        // Ensure permission booleans are set (handles race condition where
        // collectWebIntegritySignals resolved before getUserMedia)
  ...
  // Each conditional render branch used to create a separate <video> element,
  // which lost the srcObject on unmount.  We now render ONE persistent <video>
  // and re-attach the stream here if the element changes.
  ...
  // Runs every 500ms when camera feed is visible to detect blur, dark, glare
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  useEffect(() => {
    ...
    const analyzeFrame = () => {
      ...
      // ── Glare detection ──
      // If >15% of pixels are nearly white, there's likely glare
      ...
      // Convert to grayscale, then compute Laplacian variance
      ...
      // Typical sharp image: variance > 200. Blurry: < 50-100
  ...
  // Whether the camera feed should be visible behind the UI
  ...
  // Environment warning icon helper
  ...
  return (
    <div className="flex-1 relative bg-black overflow-hidden">
      ...
      {phase === 'camera-error' && (
        <div className="absolute inset-0 flex items-center justify-center p-6 z-20">
          <div className="max-w-md w-full space-y-6 text-center">
            ...
            <h2 className="text-xl font-semibold text-white">Camera Access Required</h2>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-left border border-white/10">
              <p className="text-sm font-medium text-white mb-2">How to fix:</p>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl font-semibold text-white shadow-md hover:shadow-lg transition-all"
      ...
      {phase === 'instructions' && (
        <div className="absolute inset-0 flex items-center justify-center p-6 z-20">
          <div className="max-w-md w-full space-y-6 text-center">
            <h2 className="text-2xl font-semibold text-white">{phaseLabel} Challenge</h2>
            ...
            <button
              ...
              className="w-full py-4 rounded-xl font-semibold text-white shadow-md hover:shadow-lg transition-all text-lg"
              style={{ backgroundColor: primaryColor }}
            >
              Got it — Start
            </button>
      ...
      {phase === 'face-guide' && (
        <>
          {/* Blurred + tinted overlay outside the oval */}
          <div
          ...
          {/* Environment quality warnings */}
          {envWarnings.length > 0 && (
            <div className="absolute top-4 left-4 right-4 flex flex-col gap-2 z-20">
              {envWarnings.map((w, i) => (
                <div
                  key={`${w.type}-${i}`}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium backdrop-blur-sm ${
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent px-6 pb-6 pt-20 text-center z-20">
            <p className="text-white text-base sm:text-lg font-medium mb-5">Position your face inside the oval</p>
            <button
              onClick={() => faceGuideReadyRef.current?.()}
              className="px-10 py-3.5 rounded-xl font-semibold text-white shadow-lg hover:shadow-xl transition-all text-base"
              style={{ backgroundColor: primaryColor }}
            >
              I'm Ready
            </button>
      ...
      {showCameraFeed && phase !== 'face-guide' && (
        <>
          {/* Blurred overlay outside the oval */}
          <div
            className="absolute inset-0 z-10 pointer-events-none"
            style={{
          ...
          {envWarnings.length > 0 && (
            <div className="absolute top-14 left-4 right-4 flex flex-col gap-1.5 z-20 pointer-events-none">
              {envWarnings.map((w, i) => (
                <div
                  key={`${w.type}-${i}`}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium backdrop-blur-sm w-fit ${
                    w.type === 'dark' ? 'bg-amber-500/20 text-amber-200 border border-amber-500/30'
                    : w.type === 'bright' ? 'bg-yellow-500/20 text-yellow-200 border border-yellow-500/30'
                    : 'bg-blue-500/20 text-blue-200 border border-blue-500/30'
                  }`}
          ...
          {showCountdown && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
              <div className="text-8xl font-bold text-white/90 drop-shadow-2xl animate-pulse">
                {countdownNumber}
              </div>
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent px-6 pb-5 pt-14 z-10">
            <p className="text-white text-base sm:text-lg font-medium text-center mb-3">
              {phaseLabel}
            </p>
